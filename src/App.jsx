...
function App() {
  ...
  const fetchWalletBalances = useCallback(async (address) => {
    ...
    try {
      const response = await fetch(`${VESTIGE_API_BASE}/wallets/${address}/value?network_id=0&denominating_asset_id=0`);
  ...
  const searchTokens = useCallback(async (query, isSell = false) => {
    ...
    try {
      ...
      const response = await fetch(`${VESTIGE_API_BASE}/assets/search?${params}`);
      const data = await response.json();
  ...
  const handleSellSearchInputChange = useCallback((event) => {
  ...
  const handleBuySearchInputChange = useCallback((event) => {
    const value = event.target.value;
  ...
  useEffect(() => {
    ...
    peraWallet
      .reconnectSession()
      .then((accounts) => {
        if (isMounted && accounts.length) {
          setAccountAddress(accounts[0]);
          fetchWalletBalances(accounts[0]);
        }
      })
      .catch((error) => {
        console.error("Failed to reconnect Pera session:", error);
      });
  ...
  }, [peraWallet, fetchWalletBalances]);

  // Fetch balances when account address changes
  useEffect(() => {
    if (accountAddress) {
      fetchWalletBalances(accountAddress);
    }
  }, [accountAddress, fetchWalletBalances]);

  const fetchQuote = useCallback(async (sellAssetId, buyAssetId, amount) => {
      ...
      if (data && data.quote) {
        ...
        if (sellAssetId === 0) {
          ...
          algoEquivalent = {
            ...
            label: `${buyAmountCalculated.toFixed(6)} ${buyAsset.name} ≈ ${sellAmountNum.toFixed(6)} ALGO`
          };
        } else if (buyAssetId === 0) {
          // Buying ALGO - show how much ALGO you're getting
          algoEquivalent = {
            amount: buyAmountNum,
            label: `${sellAmountNum.toFixed(6)} ${sellAsset.name} ≈ ${buyAmountNum.toFixed(6)} ALGO`
          };
        } else {
          // Neither is ALGO - show exchange rate
          algoEquivalent = {
            amount: pricePerUnit,
            label: `1 ${sellAsset.name} = ${pricePerUnit.toFixed(6)} ${buyAsset.name}`
          };
        }
  ...
  const handleSellAmountChange = useCallback((value) => {
    ...
    const timeoutId = setTimeout(() => {
      if (sellAsset.id !== undefined && buyAsset.id !== undefined && buyAsset.id !== null) {
  ...
  const getCurrentBalance = useCallback((assetId) => {
    if (!walletBalances || Object.keys(walletBalances).length === 0 || assetId === null || assetId === undefined) {
      return 0;
    }
  ...
  const handleAssetOptIn = async (assetId) => {
    ...
    try {
      const isOptedIn = await isOptedIntoAsset(accountAddress, assetId);
      if (isOptedIn) {
        return true;
      }
  ...
  const executeSwap = async () => {
    ...
    try {
      ...
      if (buyAsset.id !== 0 && buyAsset.id !== '0') {
        const isOptedIn = await isOptedIntoAsset(accountAddress, parseInt(buyAsset.id));
      ...
      const swapTxnsForPera = decodedTxns.map(item =>
        item.needsUserSignature
          ? { txn: item.txn, signers: [accountAddress] }
      ...
      for (let i = 0; i < decodedTxns.length; i++) {
        ...
        if (item.needsUserSignature) {
          ...
          if (walletSignedTxn instanceof Uint8Array) {
            ...
            finalSwapSignedTxns.push(new Uint8Array(Buffer.from(walletSignedTxn, 'base64')));
          } else {
          ...
          if (typeof blob === 'string') {
            ...
            throw new Error(`LogicSigBlob format unrecognized at transaction ${i + 1}`);
          }
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            Balance: {isLoadingBalances ? "Loading..." : getCurrentBalance(sellAsset.id).toLocaleString(undefined, {
        ...
        <div style={{
            ...
            <div style={{ position: 'relative' }}>
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <span style={{ fontSize: '16px', fontWeight: '600', color: buyAsset.name ? '#333' : '#999' }}>
          ...
          <div style={{
            ...
            Balance: {isLoadingBalances ? "Loading..." : getCurrentBalance(buyAsset.id).toLocaleString(undefined, {
              minimumFractionDigits: 0,
              maximumFractionDigits: 6
            })} {buyAsset.name}
          </div>
        ...
        {quoteInfo && (
          <div style={{
            ...
            <div style={{
              ...
              Exchange Rate: 1 {quoteInfo.sellAsset} = {quoteInfo.pricePerUnit.toFixed(6)} {quoteInfo.buyAsset}
            </div>
          </div>
        )}
        ...
        <button 
          onClick={executeSwap}
          disabled={isSwapping || !accountAddress || parseFloat(sellAmount) <= 0 || !buyAsset.name}
          style={{
            width: '100%',
            padding: '16px',
            background: isSwapping || !accountAddress || parseFloat(sellAmount) <= 0 || !buyAsset.name
              ? '#ccc' 
              : 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)',
            color: 'white',
            border: 'none',
            borderRadius: '12px',
            cursor: isSwapping || !accountAddress || parseFloat(sellAmount) <= 0 || !buyAsset.name
              ? 'not-allowed' 
              : 'pointer',
            fontSize: '18px',
            fontWeight: '700',
          ...
          onMouseDown={(e) => {
            if (!isSwapping && accountAddress && parseFloat(sellAmount) > 0 && buyAsset.name) {
        ...
        >
          {isSwapping ? "Swapping..." : buyAsset.name ? `Swap ${sellAsset.name} → ${buyAsset.name}` : "Select a token to buy"}
        </button>
                  ...
                  <div style={{
                    ...
                    <div style={{
                      ...
                      <div style={{
                        ...
                        <div style={{
                          ...
                          <span style={{ fontSize: '16px', fontWeight: '700', color: '#333' }}>
                            {parseFloat(swap.sellAmount).toLocaleString(undefined, {
                              minimumFractionDigits: 0,
                              maximumFractionDigits: 6
                            })}
                          </span>
                      ...
                      <div style={{
                        ...
                        <span style={{ fontSize: '16px', color: '#667eea', fontWeight: 'bold' }}>→</span>
                      </div>
                      ...
                      <div style={{
                        ...
                        <div style={{
                          ...
                          <span style={{ fontSize: '16px', fontWeight: '700', color: '#10b981' }}>
                            {parseFloat(swap.buyAmount).toLocaleString(undefined, {
                              minimumFractionDigits: 0,
                              maximumFractionDigits: 6
                            })}
                          </span>
